...
func RegisterAction(c *gin.Context) {
	...
	user, _ := s.Users.FindName(c, userName)
	if nil != user {
		...
		if err := s.Users.Create(c, user); nil != err {
			result.Code = errors.CodeErr
...
//		result.Code = pkg.CodeErr
//		result.Msg = "can not get user by name [" + userName + "]"
//		return
//	}
//
//	// check password
...
//		result.Code = pkg.CodeErr
//		result.Msg = "Invalid Password [" + userName + "]"
//		return
//	}
//
...
//
//	// update user action
//	if err := service.User.UpdateUserAction(user); nil != err {
...
//		data := map[string]interface{}{}
//		data["msg"] = "error"
//		data["errorMsg"] = "can not get user by name [" + userName + "]"
//		result.Result = data
//		return
//	}
//
...
//		result.Code = pkg.CodeErr
//		result.Msg = "Invalid Old Password [" + userName + "]"
//		return
//	} else {
//		user.Password = crypto.HashAndSalt([]byte(password))
//		_ = service.User.UpdateUser(user)
//	}
//
//	own := service.User.GetUser(user.ID)
//	if nil == own {
//		log.Warn().Msg("can not get user by name [" + userName + "]")
//
//		data := map[string]interface{}{}
...
//
//	user := service.User.GetUserWithRole(session.UID)
//	if nil == user {
...
//	roleModels := service.Role.ConsoleGetRoles()
